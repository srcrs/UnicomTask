...
def send(contents):
    import requests
    token = 'dd982984173d4682bfa40f4d69012d2e'  # 在pushplus网站中可以找到
    title = 'Unicom'  # 改成你要的标题内容
    content = contents  # 改成你要的正文内容
    ...
    try:
        ...
        for flow in flowList:
            ...
            if res1['code'] == '0000':
                ...
                send('【沃之树-领流量】: 4M流量 x' + str(num))
            else:
                logging.info('【沃之树-领流量】: 已领取过 x' + str(num))
                send('【沃之树-领流量】: 已领取过 x' + str(num))
            # 等待1秒钟
            time.sleep(1)
            num = num + 1
        ...
        send('【沃之树-浇水】: 获得' + str(res2['data']['addedValue']) + '培养值')
        time.sleep(1)
    except Exception as e:
        print(traceback.format_exc())
        logging.error('【沃之树】: 错误，原因为: ' + str(e))
        send('【沃之树】: 错误，原因为: ' + str(e))
...
def daySign_task(username):
    try:
        ...
        if res1['status'] == '0000':
            ...
            send('【每日签到】: ' + '打卡成功,' + res2['data']['statusDesc'])
...
def luckDraw_task():
    try:
        ...
        for i in range(3):
            ...
            send('【天天抽奖】: ' + res['RspMsg'] + ' x' + str(i + 1))
            # 等待1秒钟
            time.sleep(1)
    except Exception as e:
        print(traceback.format_exc())
        logging.error('【天天抽奖】: 错误，原因为: ' + str(e))
        send('【天天抽奖】: 错误，原因为: ' + str(e))
...
def gameCenterSign_Task(username):
    ...
    try:
        ...
        if res1['respCode'] == '0000' and res1['respDesc'] == '打卡并奖励成功':
            ...
            send('【游戏中心签到】: ' + '获得' + str(res1['currentIntegral']) + '积分')
        elif res1['respCode'] == '0000':
            logging.info('【游戏中心签到】: ' + res1['respDesc'])
            send('【游戏中心签到】: ' + res1['respDesc'])
        time.sleep(1)
        # 游戏频道积分
        ...
        if res2['code'] == '0000':
            ...
            send('【游戏频道打卡】: 获得' + str(res2['integralNum']) + '积分')
        else:
            logging.info('【游戏频道打卡】: ' + res2['msg'])
            send('【游戏频道打卡】: ' + res2['msg'])
        client.headers.pop('referer')
    ...
    try:
        for i in range(3):
            ...
            if res1['reason'] == '00':
                ...
                send('【4G流量包-看视频】: 获得' + res1['addNum'] + 'M流量 x' + str(i + 1))
            elif res1['reason'] == '01':
                logging.info('【4G流量包-看视频】: 已完成' + ' x' + str(i + 1))
                send('【4G流量包-看视频】: 已完成' + ' x' + str(i + 1))
            # 等待1秒钟
            time.sleep(1)
    except Exception as e:
        print(traceback.format_exc())
        logging.error('【4G流量包】: 错误，原因为: ' + str(e))
        send('【4G流量包】: 错误，原因为: ' + str(e))
...
def day100Integral_task():
    ...
    try:
        integral = client.post('https://m.client.10010.com/welfare-mall-front/mobile/integral/gettheintegral/v1',
                               data=data)
        integral.encoding = 'utf-8'
        res = integral.json()
        logging.info("【100定向积分】: " + res['msg'])
        send("【100定向积分】: " + res['msg'])
        time.sleep(1)
    except Exception as e:
        print(traceback.format_exc())
        logging.error('【100定向积分】: 错误，原因为: ' + str(e))
        send('【100定向积分】: 错误，原因为: ' + str(e))
...
def pointsLottery_task(n):
    try:
        ...
        for i in range(num):
            ...
            send("【积分抽奖】: " + res2['RspMsg'] + ' x' + str(i + 1))
            # 等待随机秒钟
            time.sleep(1)
    except Exception as e:
        print(traceback.format_exc())
        logging.error('【积分抽奖】: 错误，原因为: ' + str(e))
        send('【积分抽奖】: 错误，原因为: ' + str(e))
...
def dongaoPoints_task():
    ...
    try:
        # 领取积分奖励
        dongaoPoint = client.post('https://m.client.10010.com/welfare-mall-front/mobile/winterTwo/getIntegral/v1',
                                  data=data)
        dongaoPoint.encoding = 'utf-8'
        res1 = dongaoPoint.json()
        # 查询领了多少积分
        dongaoNum = client.post('https://m.client.10010.com/welfare-mall-front/mobile/winterTwo/winterTwoShop/v1',
                                data=data)
        ...
        if res1['resdata']['code'] == '0000':
            ...
            send('【东奥积分活动】: ' + res1['resdata']['desc'] + '，' + str(point) + '积分')
        else:
            logging.info('【东奥积分活动】: ' + res1['resdata']['desc'] + '，' + res2['resdata']['desc'])
            send('【东奥积分活动】: ' + res1['resdata']['desc'] + '，' + res2['resdata']['desc'])
        time.sleep(1)
    except Exception as e:
        print(traceback.format_exc())
        logging.error('【东奥积分活动】: 错误，原因为: ' + str(e))
        send('【东奥积分活动】: 错误，原因为: ' + str(e))
...
def dayOneG_Task():
    try:
        ...
        if (res1['data']['taskInfo']['status'] == '1'):
            ...
            send('【1G流量日包】: ' + res2['data']['statusDesc'])
        else:
            logging.info('【1G流量日包】: ' + res1['data']['taskInfo']['btn'])
            send('【1G流量日包】: ' + res1['data']['taskInfo']['btn'])
        time.sleep(1)
    except Exception as e:
        print(traceback.format_exc())
        logging.error('【1G流量日包】: 错误，原因为: ' + str(e))
        send('【1G流量日包】: 错误，原因为: ' + str(e))
...
def readJson():
    try:
        ...
        send('账号信息获取失败错误，原因为: ' + str(e))
        logging.error('1.请检查是否在Secrets添加了账号信息，以及添加的位置是否正确。')
        send('1.请检查是否在Secrets添加了账号信息，以及添加的位置是否正确。')
        logging.error('2.填写之前，是否在网站验证过Json格式的正确性。')
        send('2.填写之前，是否在网站验证过Json格式的正确性。')
    ...
    try:
        ...
        for r in res['resdata']['data']:
            if r['name'] != None and r['number'] != None:
                ...
                send('【' + str(r['name']) + '】: ' + str(r['number']))
        time.sleep(1)
    except Exception as e:
        print(traceback.format_exc())
        logging.error('【积分余额】: 错误，原因为: ' + str(e))
        send('【积分余额】: 错误，原因为: ' + str(e))
...
def actionFlow(username):
    ...
    for end in endTime:
        ...
        if end < 86400:
            ...
            # 激活流量包
            res = client.post('http://m.client.10010.com/myPrizeForActivity/myPrize/activationFlowPackages.htm',
                              data=activeData)
            res.encoding = 'utf-8'
            res = res.json()
            if res['status'] == '200':
